...
app.post('/fetch', async (req, res) => {
  try {
    ...
    function replaceYaleWithFale(i, el) {
      if ($(el).children().length === 0 || $(el).text().trim() !== '') {
        ...
        if (content && $(el).children().length === 0) {
          // Replace Yale with Fale in text content only (preserve case)
    ...
    $('body *').contents().filter(function() {
      ...
      // Replace text content but not in URLs or attributes (preserve case)
    ...
    // Process title separately (preserve case)
